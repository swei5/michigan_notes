...
- **Cost-based Search**
	...
	- Used for **more complicated or advanced scenarios**
...
- Cannot compare plans because there is **NO cost model** but can "direct" a transformation to a preferred plan
...
1. Logical Costs
	- Estimate **result sizes** per operator
		- Result set is the input set for the parent operator
...
- What matters is **sequential vs. Random I/O**

This is easier to model if the DBMS has full control over **buffer management**.

#### Postgres Cost Model 
Uses a combination of **CPU and I/O costs** that are weighted by some *magic* **constant factors**, which is used to represent the difference between different types of cost.
...
- **Sequential I/O is 4 x faster** than random I/O

This is a simplification - across different hardwares there are different constants. In reality it is really difficult to estimate the cost.

#### IBM DB2 Cost Model 
Consists many more aspects, encompassing
- Database characteristics in system catalogs
- Hardware environment
- Storage device characteristics
- Communications bandwidth (distributed DB only)
- Memory resources
