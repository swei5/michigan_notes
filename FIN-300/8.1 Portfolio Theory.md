...
Given the return of a portfolio $\mathbf{R}=[r_{1}, r_{2},\cdots, r_{n}]$, weight $\mathbf{w}$, and standard deviation $\mathbf{\sigma}$, the expected return on a portfolio is a **weighted average** of the expected returns on **EACH asset** in the portfolio. $$\mathbb{E}(\mathbf{R})=\sum\limits_{i} w_{i}\mathbb{E}(r_{i})$$

The variance of portfolio returns is a **weighted sum** of the **covariance of returns** on **EACH pair** of assets. $$\text{Var}(\mathbf{R})=\sum\limits_{i,j, i\ne j}w_{i}w_{j}\text{Cov}(r_{i},r_{j})+\sum\limits_{i,j, i=j}w_{i}^{2}\sigma_{i}^{2}$$
