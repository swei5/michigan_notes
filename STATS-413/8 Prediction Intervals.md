...
- Consider the case $p=0$ and sample size $n \to \infty$: then $\overline{y}$ is going to have a confidence interval of $[\overline{y}, \overline{y}]$, which is very unlikely to capture future observation due to the error term $\varepsilon_i$
...
Now, suppose the stronger linear model holds, and that we are interested in forming a **prediction interval** for a future observation $y^\star$ with covariate values $\tilde{x}$. $$y^{\star}=\beta_{0}+\beta_{1} \tilde{x}_{1}+\dots+\beta_{p}\tilde{x}_{p}+\varepsilon^{\star}$$ and $$ \varepsilon_{1},\dots,\varepsilon_{n}, \varepsilon^{\star} \sim N (0,\sigma_{\varepsilon}^{2}) $$
Here, $$\mathbb{E}(y^\star)=\mathbb{E}(\tilde{x}^{T}\beta+\varepsilon^{\star})=\tilde{x}^{T}\beta$$ and $$\text{Var}(y^{\star})=\sigma_{\varepsilon}^2$$
...
Then, we would construct a valid $100 (1-\alpha)\%$ prediction interval for a future observation at $\tilde{x}$: $$\tilde{x}^{T} \beta \pm z_{1-\alpha/2}\sigma_{\varepsilon}$$
...
2. Has a standard deviation of $\sigma_{\varepsilon}$ (homoskedasticity)
...
- We don't know $\sigma_{\varepsilon}$ - crucial for the spread
...
Using the independence of $y^{\star}$ and $\hat{\mu}_{y|\tilde{x}}$ and our results on **[[7 Inferences#^99db9f|linear combinations of slopes]]**, we can derive a variance to help determine the width of our prediction intervals: $$\begin{align}\text{Var}(y^{\star}-\hat{\mu}_{y|\tilde{x}})&=\text{Var}(y^{\star})+\text{Var}(\hat{\mu}_{y|\tilde{x}}) \\ &= \sigma_{\varepsilon}^{2}+\sigma_{\varepsilon}^{2} \tilde{x}^{T}(X^{T}X)^{-1}\tilde{x} \\ &= \sigma_{\varepsilon}^{2}(1+\tilde{x}^{T}(X^{T}X)^{-1}\tilde{x})\end{align}$$
Drawing conclusion back [[#^0527d8|here]], we have that $$\frac{y^{\star}-\hat{\mu}_{y|\tilde{x}}}{\sigma_{\varepsilon} \sqrt{(1+\tilde{x}^{T}(X^{T}X)^{-1}\tilde{x})}} \sim N(0,1)$$
Since, again, the value of $\sigma_{\varepsilon}$ is unknown to us, so we will use estimate it using RMSE: $$\hat{\sigma}_{\varepsilon}= \sqrt{\frac{1}{n-p-1} \sum_{i=1}^{n} e^{2}}$$ which is the standard deviation of the residuals.

Putting it all together, $$T=\frac{y^{\star}-\hat{\mu}_{y|\tilde{x}}}{\hat{\sigma}_{\varepsilon} \sqrt{(1+\tilde{x}^{T}(X^{T}X)^{-1}\tilde{x})}}  \sim t_{n-p-1}$$ under the stronger linear model. Based on this statistic, an $100 (1-\alpha)\%$ prediction interval for a covariate $\tilde{x}$ is $$\hat{\mu}_{y|\tilde{x}} \pm t_{1-\alpha/2,n-p-1}\hat{\sigma}_{\varepsilon} \sqrt{(1+\tilde{x}^{T}(X^{T}X)^{-1}\tilde{x})}$$ where $\hat{\sigma}_{\varepsilon} \sqrt{(1+\tilde{x}^{T}(X^{T}X)^{-1}\tilde{x})}$ is the standard error for the predicted response $\hat{\mu}_{y|\tilde{x}}$.
...
- $\hat{\text{Var}}(y^{\star}|\tilde{x})=\hat{\sigma}_{\varepsilon}^{2}$

If the distribution of $y$ is in fact heteroskedastic, however, then 

![[Pasted image 20240921142230.png|400]]

The $\hat{\sigma}_{\varepsilon}$ we computed **may NOT** be a good estimate of the true standard deviation $\sigma_{\varepsilon}$, as in this example $\hat{\sigma}_{\varepsilon}$ may be too large for smaller $y$ values and vice versa as we assumed constant variances.

##### Approximation of Prediction Interval
An approximation to the prediction intervals produced by `R` (appropriate when $n$ is **large**) is given by $$\hat{\mu}_{y|\tilde{x}} \pm z_{1-\alpha/2}\hat{\sigma}_{\varepsilon}$$ which ignores the **randomness** in $\hat{\sigma}_{\varepsilon}$ and randomness in $\tilde{x}^{T}\hat{\beta}$.

When $n \to \infty$, $\hat{\sigma}_{\varepsilon} \to \sigma_{\varepsilon}$ and randomness in the center of interval is **small** relative to $\sigma_{\varepsilon}$.
...
- $50\%$ of future responses will fall within $\pm 2/3\hat{\sigma}_{\varepsilon}$ from their predicted value
- $68\%$ of future responses will fall within $\pm 1\hat{\sigma}_{\varepsilon}$ from their predicted value
- $95\%$ of future responses will fall within $\pm 2\hat{\sigma}_{\varepsilon}$ from their predicted value 
- $99.7\%$ of future responses will fall within $\pm 3\hat{\sigma}_{\varepsilon}$ from their predicted value
```
...
(1) **Prediction Interval** for future observations $y^\star$: $$\hat{\mu}_{y|\tilde{x}} \pm t_{1-\alpha/2,n-p-1}\hat{\sigma}_{\varepsilon} \sqrt{(1+\tilde{x}^{T}(X^{T}X)^{-1}\tilde{x})}$$
(2) **Confidence Interval** for ${\mu}_{y|\tilde{x}}$: $$\hat{\mu}_{y|\tilde{x}} \pm t_{1-\alpha/2,n-p-1}\hat{\sigma_{\varepsilon}}\sqrt{\tilde{x}^{T}(X^TX)^{-1}\tilde{x}}$$

Both have the **same center**, but **different width.** Prediction intervals are wider (often much wider) than confidence intervals.

**Observations**
- Variance of $\hat{\mu}_{y|\tilde{x}}$, $\sigma_{\varepsilon}^{2} \tilde{x}^{T}(X^{T}X)^{-1}\tilde{x}$ tends to **zero** as $n \to \infty$
- Variance used for **prediction intervals**, $\sigma_{\varepsilon}^{2}(1+\tilde{x}^{T}(X^{T}X)^{-1}\tilde{x})$, tends to **one** as $n \to \infty$
	- This is effectively saying that, even if we knew $\beta$ (and hence $\mu_{y|\tilde{x}}$ and hence my confidence interval for predicted mean response is just a point), we still **CAN'T** perfectly predict future observations, given the variances introduced by the error $\varepsilon_i$
